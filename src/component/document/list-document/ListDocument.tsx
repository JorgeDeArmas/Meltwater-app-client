...
const ListDocument = () => {
   ...
   useEffect(() => {
      fetchDocuments(state.pagination.page, limitPerPage, state.search.criteria).catch((error) =>
   ...
   return (
      <div className="doc-dashboard-container">
         <div className="d-flex align-items-center mx-2  mt-2">
            ...
            <button className="btn btn-sm btn-outline-success" onClick={onShowCreateModalHandler}>
               Upload Document
            </button>
         </div>
         ...
         {state.actions.loading &&
            ReactDOM.createPortal(
               ...
               document.getElementById("loading-root") as Element,
            )}
      </div>
   );
};
